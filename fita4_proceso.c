...
int			newFd;
int 		result;                      
int h=0;                          //Muestra el cambio de estado
...
char missatge_arduino2[255];      //Protocolo que se guardar para enviar al arduino                            
...
int nmediana=1;                   //Número de muestras con el que se quiere hacer la muestra
...
void* codi_fill(void* parametre){ // codi thread fill
	...
	while(1){ 
        ...
        if (h==1 && h_anterior==0){
            strcpy(missatge_arduino, missatge_arduino2);
...
float Media(int numero) {
    ...
    if (numero!=0) 	m = suma/numero;

	else  {
        printf("ERROR EN MEDIA DIVISION ENTRE 0\n");
        m=0;
    }
	return m;
...
void paro(){
        ...
        sprintf(missatge_arduino2, "AM000Z");
        sprintf(missatge,"{M0}");
 }
...
void marcha(){
    ...
    char tiempo_entrado_usuario[3];

        sprintf(tiempo_entrado_usuario,"%c%c",buffer[3],buffer[4]);
        tiempo_muestreo=atoi(tiempo_entrado_usuario);

        h=1;

        sprintf(missatge_arduino2, "AM1%dZ",tiempo_muestreo);
        sprintf(missatge,"{M0}");
}
